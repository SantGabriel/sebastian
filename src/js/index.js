...
function isCandidatura(jobId) {
    ...
    return candidaturas.includes(jobId);
}
    ...
    container.innerHTML = jobs.map((job, idx) => {
        ...
        const candidaturaHtml = job.candidatura
            ? `<div class="candidatura-aviso">
              ...
              ${job.candidatura.email ? `<a href="${job.candidatura.email}" target="_blank"><i class="fa-solid fa-arrow-up-right-from-square" style="font-size:11px"></i> Abrir link</a>` : ''}
            </div>`
            : '';
        ...
        const cityWarningHtml = job.cidadeVaga && modalidade && modalidade !== 'Remoto' && candidateLocation && cityFromVaga !== cityFromCandidate
            ? `<div class="city-warning"><i class="fa-solid fa-triangle-exclamation"></i>Vaga <strong>${modalidade}</strong> em <strong>${job.cidadeVaga}</strong> — sua localização cadastrada é <strong>${candidateLocation}</strong>.</div>`
            : '';

        const cvBtn = hasCv
            ? cvAuthorized
                ? `<a class="btn btn-cv" href="src/pages/cv.html?job=${job.id}" target="_blank"><i class="fa-solid fa-file-pdf"></i> CV</a>`
                : `<span class="btn btn-pending"><i class="fa-solid fa-lock"></i> CV</span>`
            : '';

        const clBtn = hasCl
            ? clAuthorized
                ? `<a class="btn btn-cl" href="src/pages/cl.html?job=${job.id}" target="_blank"><i class="fa-solid fa-envelope"></i> Cover Letter</a>`
          ...
          <div class="job-card">
            <div class="card-header">
              <div class="card-header-left">
                <h2><span style="font-weight:normal;margin-right:6px">${idx+1}.</span>${job.vaga}${job.link ? `<a href="${job.link}" target="_blank" class="vaga-link"><i class="fa-solid fa-arrow-up-right-from-square" style="font-size:11px"></i> Abrir</a>` : ''}</h2>
                <div class="company"><i class="fa-solid fa-building"></i> ${job.empresa}${job.modalidade ? ` <span style="font-size:11px;font-weight:600;background:#e8f0fe;color:#1a56db;border-radius:10px;padding:2px 8px;margin-left:6px">${job.modalidade}</span>` : ''}${job.contratacao ? ` <span style="font-size:11px;font-weight:600;background:#e8f0fe;color:#1a56db;border-radius:10px;padding:2px 8px;margin-left:6px">${job.contratacao}</span>` : ''}</div>
              </div>
            ...
            <label class="candidato-checkbox ${isCandidatura(job.id) ? 'marcado' : ''}" id="candidato-container-${job.id}">
              <input type="checkbox" id="candidato-${job.id}" ${isCandidatura(job.id) ? 'checked' : ''} onchange="toggleCandidatura('${job.id}')" />
